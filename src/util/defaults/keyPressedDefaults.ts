...
const keyPressed = ({ p5, event, os, fileName, seed, width, dimensions, background, renderSVG }: KeyPressed) => {
  if (os === 'mac') {
    if (event.key === 's' && event.metaKey) {
      event.preventDefault();
      seed && p5.randomSeed(seed);
      seed && p5.noiseSeed(seed);
      const ratio = ((dimensions && dimensions[0]) ?? width ?? p5.width) / p5.width;
