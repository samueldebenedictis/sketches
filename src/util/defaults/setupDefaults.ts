...
const setupDefaults = ({
  ...
  const usedHeight = dimensions ? dimensions[1] : height ? height : p5.windowHeight;
  ...
  const paddingHeight =
    padding && padding.length === 2 ? padding[1] * 2 : padding && padding.length === 1 ? padding[0] * 2 : 0;
  ...
  if (usedWidth > p5.windowWidth || usedHeight > p5.windowHeight) {
    if (aspectRatio > windowRatio) {
      ...
      p5.createCanvas(maxWidth, newHeight, renderSVG ? p5.SVG : renderer).parent(canvasParentRef);
    } else {
      const newWidth = Math.round(maxHeight * aspectRatio);
      p5.createCanvas(newWidth, maxHeight, renderSVG ? p5.SVG : renderer).parent(canvasParentRef);
    }
  } else {
    p5.createCanvas(usedWidth, usedHeight, renderSVG ? p5.SVG : renderer).parent(canvasParentRef);
  }
